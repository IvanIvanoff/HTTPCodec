...
namespace httpcodec {
    ...
    struct RequestHeaderGrammar : qi::grammar<Iterator, RequestHeader(), Skipper> {
        RequestHeaderGrammar() : RequestHeaderGrammar::base_type(http_header, "RequestHeader Grammar") {
            method = +qi::alpha;
            uri = +qi::graph;
...
// Response header parser
    ...
    struct ResponseHeaderGrammar : qi::grammar<Iterator, ResponseHeader(), Skipper> {
        ResponseHeaderGrammar() : ResponseHeaderGrammar::base_type(http_header, "ResponseHeader Grammar") {
            http_ver = "HTTP/" >> +qi::char_("0-9.");
    ...
    private:
        ...
        // lexemes
        qi::rule<Iterator, std::string()> http_ver, status_code, status_message;
        qi::rule<Iterator, std::string()> field_key, field_value;
    };
}
